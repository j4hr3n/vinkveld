<script lang="ts">
    ...
    const fieldInputClass =
        "w-full py-3 px-4 border-[1.5px] border-cream-dark rounded-xl text-base font-[inherit] transition-all duration-300 bg-cream/60 focus:outline-none focus:border-wine-light focus:bg-white focus:shadow-[0_0_0_3px_rgba(92,26,42,0.06)]";
    const fieldLabelClass =
        "block text-[0.82rem] font-medium text-text-light mb-2 uppercase tracking-wider";
</script>

<div
    class="relative bg-white/80 backdrop-blur-sm rounded-2xl p-7 max-[480px]:p-5 shadow-[0_4px_24px_rgba(92,26,42,0.06)] border border-cream-dark/60 animate-rise-in overflow-hidden"
>
    <!-- Top accent line -->
    <div
        class="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-wine/30 to-transparent"
    ></div>

    <h3 class="text-lg text-wine mb-6 font-semibold font-serif">
        {editing ? "Rediger vin" : "Legg til vin"}
    </h3>
    ...
    <div class="mb-5">
        ...
        <div class="grid grid-cols-4 gap-3">
            {#each COLORS as color}
                <button
                    type="button"
                    class="flex flex-col items-center gap-1.5 py-2.5 px-3 rounded-xl border-[1.5px] cursor-pointer transition-all duration-300 font-[inherit] {selectedColor ===
                    color.id
                        ? 'border-wine bg-white shadow-[0_2px_8px_rgba(92,26,42,0.1)]'
                        : 'border-cream-dark bg-cream/40 hover:border-wine-light hover:bg-white/60'}"
                    onclick={() => (selectedColor = color.id as Wine["color"])}
                >
                    <span
                        class="w-5 h-5 rounded-full transition-transform duration-300 {selectedColor ===
                        color.id
                            ? 'scale-110'
                            : ''}"
                    ...
                    ></span>
                    <span
                        class="text-[0.75rem] font-medium {selectedColor ===
                        color.id
                            ? 'text-wine'
                            : 'text-text-light'}">{color.label}</span
                    >
                </button>
            {/each}
        </div>
    ...
    <div class="mb-5">
        <label for="wine-link" class={fieldLabelClass}
            >Lenke <span
                class="normal-case tracking-normal font-normal opacity-60"
                >(valgfritt)</span
            ></label
        >
        <input
            type="url"
            id="wine-link"
            placeholder="f.eks. https://vinmonopolet.no/..."
            bind:value={link}
            class={fieldInputClass}
        />
    </div>
    <div class="mb-7">
        <label for="wine-notes" class={fieldLabelClass}
            >Notater <span
                class="normal-case tracking-normal font-normal opacity-60"
                >(valgfritt)</span
            ></label
        ...
        <input
            type="text"
            id="wine-notes"
            placeholder="f.eks. Fra kjelleren"
            bind:value={notes}
            onkeydown={handleKeydown}
            class={fieldInputClass}
        />
